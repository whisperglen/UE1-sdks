...
public:
	...
	void Empty( INT Slack=0 )
	{
		...
		if( GUndo )
			GUndo->SaveArray( Owner, this, 0, FArray::ArrayNum, -1, sizeof(T), SerializeItem, DestructItem );
	...
	INT AddUniqueItem( const T& Item )
	{
		...
		for( INT Index=0; Index< FArray::ArrayNum; Index++ )
			if( (*this)(Index)==Item )
				return Index;
		return AddItem( Item );
		unguardSlow;
	...
	INT RemoveItem( const T& Item )
	{
		...
		INT OriginalNum= FArray::ArrayNum;
		for( INT Index=0; Index< FArray::ArrayNum; Index++ )
			if( (*this)(Index)==Item )
				Remove( Index-- );
		return OriginalNum - FArray::ArrayNum;
		unguardSlow;
	}
	...
	void ModifyAllItems()
	{
		...
		if( GUndo )
			GUndo->SaveArray( Owner, this, 0, FArray::Num(), 0, sizeof(T), SerializeItem, DestructItem );
		unguardSlow;
	}
...
public:
	...
	void MultiFind( const TK& Key, TArray<TI>& Values ) 
	{
		...
		for( INT i= TMapBase<TK, TI>::Hash[(GetTypeHash(Key) & (TMapBase<TK, TI>::HashCount-1))]; i!=INDEX_NONE; i=Pairs(i).HashNext )
			if( Pairs(i).Key==Key )
	...
	{
		for( INT i= TMapBase<TK, TI>::Hash[(GetTypeHash(InKey) & (TMapBase<TK, TI>::HashCount-1))]; i!=INDEX_NONE; i=Pairs(i).HashNext )
	...
	INT RemovePair( typename TTypeInfo<TK>::ConstInitType InKey, typename TTypeInfo<TI>::ConstInitType InValue )
	{
		...
		INT Count=0;
		for( INT i= TMapBase<TK, TI>::Pairs.Num()-1; i>=0; i-- )
			if(TMapBase<TK, TI>::Pairs(i).Key==InKey && TMapBase<TK, TI>::Pairs(i).Value==InValue )
				{TMapBase<TK, TI>::Pairs.Remove(i); Count++;}
		if( Count )
